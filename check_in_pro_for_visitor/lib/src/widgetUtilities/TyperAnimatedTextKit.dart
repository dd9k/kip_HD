...
class _TyperState extends State<TyperAnimatedTextKit> with TickerProviderStateMixin {
    ...
    for (int i = 0; i < widget.text.length; i++) {
      try {
        ...
        _texts.add({
          ...
          'speed': widget.text[i].containsKey('speed') ? widget.text[i]['speed'] : _speed,
  ...
  void _nextAnimation() {
    ...
    _typingText = StepTween(begin: 0, end: _texts[_index]['text'].length).animate(_controller)
  ...
  void _onTap() {
    ...
    if (widget.displayFullTextOnTap) {
      if (_isCurrentlyPausing) {
        ...
        left = _texts[_index]['speed'].inMilliseconds * (_texts[_index]['text'].length - _typingText.value);
