...
class _RotatingTextState extends State<FadeAnimatedTextKit> with TickerProviderStateMixin {
    ...
    for (int i = 0; i < widget.text.length; i++) {
      try {
        ...
        _texts.add({
          ...
          'pause': widget.text[i].containsKey('pause') ? widget.text[i]['pause'] : _pause
        });
      } catch (e) {
  ...
  @override
  void didUpdateWidget(FadeAnimatedTextKit oldWidget) {
  ...
  void _nextAnimation() {
    ...
    if (isLast) {
      if (widget.isRepeatingAnimation && (_currentRepeatCount != (widget.totalRepeatCount - 1))) {
        _index = 0;
        _currentRepeatCount++;
      } else {
    ...
    _fadeIn = Tween<double>(begin: 0.0, end: 1.0)
        .animate(CurvedAnimation(parent: _controller, curve: Interval(0.0, 0.5, curve: Curves.linear)));

    _fadeOut = Tween<double>(begin: 1.0, end: 0.0)
        .animate(CurvedAnimation(parent: _controller, curve: Interval(0.8, 1.0, curve: Curves.linear)))
