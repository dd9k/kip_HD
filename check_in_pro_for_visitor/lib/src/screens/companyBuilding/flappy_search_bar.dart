...
class SearchBar<T> extends StatefulWidget {
  ...
  SearchBar({
    ...
    this.listPadding = const EdgeInsets.all(20),
...
//    });
  ...
  Widget _buildListView(List<T> items, Widget Function(T item, int index) builder) {
    var listWidget = SingleChildScrollView(
      child: Container(
        color: Colors.white,
        height:
            items.length >= 6 ? MediaQuery.of(context).size.height * 0.89 : MediaQuery.of(context).size.height * 0.5,
        child: Padding(
            padding: widget.listPadding,
            child: GridView.count(
              shrinkWrap: widget.shrinkWrap,
              crossAxisCount: items.length >= 6 ? widget.crossAxisCount : 1,
  ...
  Widget _buildContent(BuildContext context) {
    ...
    } else if (_searchQueryController.text.length > 0 && _list.isEmpty) {
      return Container(
        color: Colors.white,
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: <Widget>[
        Padding(
          padding: isShowSearch ? EdgeInsets.all(0.0) : EdgeInsets.only(left: 58.0 * SizeConfig.devicePixelRatio),
          child: Container(
            height: 80,
            alignment: Alignment.topRight,
            color: isShowSearch ? Colors.white : Colors.transparent,
            child: Row(
              mainAxisAlignment: MainAxisAlignment.end,
              crossAxisAlignment: isShowSearch ? CrossAxisAlignment.center : CrossAxisAlignment.start,
              children: <Widget>[
                GestureDetector(
                  onTap: _cancel,
                  child: AnimatedOpacity(
                    opacity: isShowSearch ? 1.0 : 0,
                    curve: Curves.easeIn,
                    duration: Duration(milliseconds: _animate ? 10 : 0),
                    child: AnimatedContainer(
                      duration: Duration(milliseconds: 20),
                      width: isShowSearch ? MediaQuery.of(context).size.width * .1 : 0,
                      child: isShowSearch
                          ? Container(
                ...
                Flexible(
                  child: AnimatedContainer(
                    duration: Duration(milliseconds: 20),
                    width: isShowSearch ? widthMax * .8 : 0.0,
                ...
                GestureDetector(
                  ...
                  child: isShowSearch
                    ...
                    height: 40,
                    width: 40,
//                    margin: EdgeInsets.only(right: 20.0, left: 0.0, top: 15.0, bottom: 0.0),
...
//        Expanded(
//          flex: 1,
//          child: isShowSearch ? _buildContent(context) : Container(),
//        ),
      ],
    );
  }
